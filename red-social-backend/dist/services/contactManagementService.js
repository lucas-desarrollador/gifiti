...
class ContactManagementService {
    ...
    static async deleteContact(userId, contactId) {
        const transaction = await models_1.Contact.sequelize.transaction();
        try {
     ...
     */
    static async cancelWishReservationsBetweenUsers(user1Id, user2Id, transaction) {
        ...
        const reservedWishes = await models_1.Wish.findAll({
            where: {
                [sequelize_1.Op.or]: [
        ...
        for (const wish of reservedWishes) {
            ...
            const reserver = await models_1.User.findByPk(wish.reservedBy, {
                attributes: ['id', 'nickname', 'realName'],
                transaction
            });
            // Cancelar la reserva
     ...
     */
    static async handleAccountDeletion(userId) {
        const transaction = await models_1.User.sequelize.transaction();
        try {
            ...
            await models_1.Contact.destroy({
                where: {
                    [sequelize_1.Op.or]: [
                        { userId: userId },
                        { contactId: userId }
     ...
     */
    static async cancelAllUserWishReservations(userId, transaction) {
        ...
        const ownedWishes = await models_1.Wish.findAll({
            where: {
                userId: userId,
                isReserved: true
            },
...
//# sourceMappingURL=contactManagementService.js.map